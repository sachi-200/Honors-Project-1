 ...
 *
 * The micro-kernel is 6 rows by 32 columns (4 AVX vectors). This requires
 * 6x4 = 24 AVX registers for the C accumulators, which is not feasible. Instead,
 * we use a 6x16 (2 AVX vectors) block, requiring 12 registers, which fits.
 *
 * Optimization stack:
 * 1.  **Outer Tiling (Macro-Kernel):** For L2/L3 cache locality.
 * 2.  **i,k,j Loop Reordering:** Maximizes temporal locality for A's tiles.
 ...
 * intrinsics and unrolled to expose instruction-level parallelism.
 *
 * @author Gemini
 * @date 2025-10-16
 */
 ...
 * It assumes A is contiguous in rows and B is packed contiguously.
...
void micro_kernel(const float* A, const float* B, float* C, int N, int KC) {
    ...
    for (int k = 0; k < KC; ++k) {
        ...
        __m256 b_vec0 = _mm256_load_ps(&B[k*NR + 0]);
        __m256 b_vec1 = _mm256_load_ps(&B[k*NR + 8]);

        // For each row of A, broadcast the element and perform FMA on the C accumulators
        for(int i = 0; i < MR; ++i) {
            __m256 a_vec = _mm256_broadcast_ss(&A[i*KC + k]);
...
void perform_tiled_multiplication(const float* A, const float* B, float* C, int N) {
    ...
    for (int jj = 0; jj < N; jj += TILE_SIZE) {
        for (int kk = 0; kk < N; kk += TILE_SIZE) {
             ...
             // Note: B is packed differently now to match the micro-kernel's access.
             // It's packed as KCxNR blocks.
                            ...
                            for (int k = kk; k < std::min(kk + TILE_SIZE, N); ++k) {
                                for(int i_s = i; i_s < std::min(i + MR, N); ++i_s) {
                                    for(int j_s = j; j_s < std::min(j + NR, N); ++j_s) {
                                        C[i_s*N + j_s] += A[i_s*N + k] * B_tile[k-kk][j_s-jj];
                                    }
                                }
                            }
                        }
