    ...
    for (int jj = 0; jj < N; jj += TILE_SIZE) {
        for (int ii = 0; ii < N; ii += TILE_SIZE) {
            for (int kk = 0; kk < N; kk += TILE_SIZE) {
                ...
                // CORRECTION: Zero the tile to prevent using garbage data in padding.
                memset(B_tile, 0, sizeof(B_tile));
                int k_max = std::min(TILE_SIZE, N - kk);
                int j_max = std::min(TILE_SIZE, N - jj);
                for (int k_local = 0; k_local < k_max; ++k_local) {
                    for (int j_local = 0; j_local < j_max; ++j_local) {
                        B_tile[k_local][j_local] = B[(kk + k_local) * N + (jj + j_local)];
                    }
                }

                // --- Inner loops iterate over the micro-tiles ---
                // CORRECTION: Swapped ii and kk loops for better C register reuse.
                for (int i = ii; i < std::min(ii + TILE_SIZE, N); i += MR) {
                    for (int j = jj; j < std::min(jj + TILE_SIZE, N); j += NR) {
                        ...
                        if (i + MR <= N && j + NR <= N && kk + TILE_SIZE <= N) {
