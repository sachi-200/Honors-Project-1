...
static void gemm_avx2_micro_kernel_impl(const float* A_block_ptr, int A_block_ld, 
    ...
    for (int k_idx = 0; k_idx < k_block_len; k_idx += UNROLL_K) {
        ...
        for (int uk = 0; uk < UNROLL_K; ++uk) {
            ...
            if (k_loop_idx + PREFETCH_DISTANCE_K_MICRO < k_block_len) {
                const float* prefetch_a_ptr = &A_block_ptr[static_cast<std::size_t>(k_loop_idx) * A_block_ld + m_offset_in_tile]; // Prefetch the block for current k_loop_idx
            ...
            for (int r = 0; r < MR_AVX2; ++r) {
                if (r < mr_actual) { // Only for valid M rows
                    ...
                    // This access for `a_val` is sequential as 'r' changes, which is cache-efficient if A_block_ld is small.
...
static void gemm_avx512_micro_kernel_impl(const float* A_block_ptr, int A_block_ld, 
    ...
    for (int k_idx = 0; k_idx < k_block_len; k_idx += UNROLL_K) {
        for (int uk = 0; uk < UNROLL_K; ++uk) {
            ...
            if (k_loop_idx + PREFETCH_DISTANCE_K_MICRO < k_block_len) {
                const float* prefetch_a_ptr = &A_block_ptr[static_cast<std::size_t>(k_loop_idx) * A_block_ld + m_offset_in_tile];
...
void gemm_avx2(const float* A, const float* B, float* C,
    ...
    for (int m_block_start = 0; m_block_start < M; m_block_start += BM) {
        for (int n_block_start = 0; n_block_start < N; n_block_start += BN) {
            ...
            // NOTE: Allocating with BM * BK, BK * BN, and BK ensures enough space
            // for the dynamically chosen BM, BN, BK from autotune.
            ...
            for (int k_block_start = 0; k_block_start < K; k_block_start += BK) {
                ...
                // Using _MM_HINT_T2 to suggest prefetching to L3 cache for larger blocks, anticipating a longer latency.
                if (k_block_start + BK < K) {
                    ...
                    int floats_per_cache_line = 64 / sizeof(float); // Typically 16 floats (for 4-byte floats)
                    
                    // Prefetch a few lines from the beginning of the next A block
                    for(int i = 0; i < std::min(current_BM, 4); ++i) { 
                        _mm_prefetch((const char*)&next_A_k_block_start_ptr[static_cast<std::size_t>(i) * lda], _MM_HINT_T2); 
                        if (current_BK > floats_per_cache_line) { // Prefetch a second cache line if block is wide enough
                            _mm_prefetch((const char*)&next_A_k_block_start_ptr[static_cast<std::size_t>(i) * lda + floats_per_cache_line], _MM_HINT_T2);
                        }
                    }
                    // Similar for B block
                    for(int i = 0; i < std::min(current_BK, 4); ++i) { 
                        _mm_prefetch((const char*)&next_B_k_block_start_ptr[static_cast<std::size_t>(i) * ldb], _MM_HINT_T2);
                        if (current_BN > floats_per_cache_line) { // Prefetch a second cache line if block is wide enough
                            _mm_prefetch((const char*)&next_B_k_block_start_ptr[static_cast<std::size_t>(i) * ldb + floats_per_cache_line], _MM_HINT_T2);
                        }
                    }
                }

                // Pack current A block (current_BM x current_BK) into column-major memory within A_packed.
                // A_packed[k_idx][i_idx] (conceptually) stores A[m_block_start + i_idx][k_block_start + k_idx].
                // The packing strategy is to iterate over rows of the A block, copy each row contiguously from original A
                // to a temporary buffer (A_row_temp), then scatter elements from A_row_temp into the column-major A_packed.
                for (int i_idx = 0; i_idx < current_BM; ++i_idx) { 
                    // Prefetch the next row of the original A matrix for efficient packing.
                    // This targets a contiguous segment in A's row-major layout.
                    ...
                    // Read a full row of A into a temporary buffer (A_row_temp). This is a contiguous read from original A.
                    // This is more efficient than strided reads directly into A_packed.
                    ...
                    // so the performance impact is minimal compared to the gain from contiguous A reads.
...
void gemm_avx512(const float* A, const float* B, float* C,
    ...
    for (int m_block_start = 0; m_block_start < M; m_block_start += BM) {
        for (int n_block_start = 0; n_block_start < N; n_block_start += BN) {
            ...
            // Allocating with BM * BK, BK * BN, and BK for the dynamically chosen BM, BN, BK.
            AlignedFloatVector A_packed_vec(static_cast<std::size_t>(BM) * BK);
            AlignedFloatVector B_packed_vec(static_cast<std::size_t>(BK) * BN);
            AlignedFloatVector A_row_temp_vec(static_cast<std::size_t>(BK)); 
            ...
            for (int k_block_start = 0; k_block_start < K; k_block_start += BK) {
                ...
                if (k_block_start + BK < K) {
                    ...
                    int floats_per_cache_line = 64 / sizeof(float);
                    
                    for(int i = 0; i < std::min(current_BM, 4); ++i) { 
                        _mm_prefetch((const char*)&next_A_k_block_start_ptr[static_cast<std::size_t>(i) * lda], _MM_HINT_T2);
                        if (current_BK > floats_per_cache_line) {
                            _mm_prefetch((const char*)&next_A_k_block_start_ptr[static_cast<std::size_t>(i) * lda + floats_per_cache_line], _MM_HINT_T2);
                        }
                    }
                    for(int i = 0; i < std::min(current_BK, 4); ++i) { 
                        _mm_prefetch((const char*)&next_B_k_block_start_ptr[static_cast<std::size_t>(i) * ldb], _MM_HINT_T2);
                        if (current_BN > floats_per_cache_line) {
                            _mm_prefetch((const char*)&next_B_k_block_start_ptr[static_cast<std::size_t>(i) * ldb + floats_per_cache_line], _MM_HINT_T2);
                        }
                    }
                }
...
int main(int argc, char* argv[]) {
    ...
    if (run_autotune) {
        ...
        // Candidates chosen to be multiples of micro-kernel dimensions (MR, NR_REG_BLOCK) where sensible.
        // Adjusted candidates, favoring smaller blocks for BM and BN to better fit L1/L2 and reduce packing overhead
        // Also adjusted BK candidates for better L2/L3 utilization.
        std::vector<int> bm_candidates = {48, 60, 72, 84, 96, 108, 120}; // Multiples of 6 (AVX2 MR)
        std::vector<int> bn_candidates = {64, 80, 96, 128, 160, 192};    // Multiples of 16 (AVX2 NR_REG_BLOCK)
        std::vector<int> bk_candidates = {128, 256, 384, 512, 768, 1024};      // Focus on L2/L3 sizes
        ...
        // Use a slightly larger problem size for autotuning to be more representative of main problem,
        // but still small enough to complete tuning quickly.
        int tune_M = 192;
        int tune_N = 192;
        int tune_K = 192;
        int tune_lda = tune_K;
        ...
        for (int cur_bm : bm_candidates) {
            for (int cur_bn : bn_candidates) {
                for (int cur_bk : bk_candidates) {
                    ...
                    for (int w = 0; w < 3; ++w) { // Increased warmup runs
                    ...
                    int num_measurements = 5; // Increased measurement runs for better average
