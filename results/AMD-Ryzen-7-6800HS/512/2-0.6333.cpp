...
//
// With BM=96, BN=128, BK=256 (default values):
...
// Total working set for one K-block iteration (A_packed + B_packed + C_tile_size): ~272KB.
        ...
        if (r < mr_actual) { // Only load from C for valid rows in current C micro-block
            if (!is_first_k_block) {
                for (int v_idx = 0; v_idx < NC_VECS_AVX2; ++v_idx) {
                    ...
                    // Masked-out elements are zeroed in the result because we initialized c_acc to zero above.
...
// based on CPU feature detection. Output messages are static to prevent spam during autotuning.
void gemm(const float* A, const float* B, float* C,
          int M, int N, int K,
          int lda, int ldb, int ldc) {

    static bool avx512_printed = false;
    static bool avx2_printed = false;
    static bool scalar_printed = false;
...
int main(int argc, char* argv[]) {
    ...
    if (run_autotune) {
        ...
        std::vector<int> bm_candidates = {64, 96, 128};
        std::vector<int> bn_candidates = {64, 128, 192};
        std::vector<int> bk_candidates = {128, 256, 384};
