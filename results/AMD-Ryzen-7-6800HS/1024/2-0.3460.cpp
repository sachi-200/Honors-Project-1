...
// Prefetch distance for B within the micro-kernel's K-loop.
// This aims to bring future B data into L1/L2 cache slightly ahead of time.
constexpr int PREFETCH_DISTANCE_K_MICRO = 4; // Prefetch 4 K-steps ahead
...
// AVX2 micro-kernel: Computes a (MR_AVX2 x NR_REG_BLOCK_AVX2) block of C.
// This micro-kernel handles the innermost loops, accumulating into registers.
// It also handles N-dimension tails using masked loads/stores across multiple vectors.
//
// Parameters:
//   A_block_ptr: Pointer to the start of the current packed A block (row-major: BM x BK).
//   A_block_ld:  Leading dimension of the packed A block (BK).
//   B_block_ptr: Pointer to the start of the current packed B block (row-major: BK x BN).
//   B_block_ld:  Leading dimension of the packed B block (BN).
//   C_ptr_at_tile: Pointer to the top-left of the current C tile in the original C matrix.
//   C_full_ld:   Leading dimension of the original C matrix (ldc).
//   m_offset_in_tile: Row offset within the current M-block/tile for this micro-kernel.
//   n_offset_in_tile: Column offset within the current N-block/tile for this micro-kernel.
//   k_block_len: The K-dimension of the current packed A/B blocks.
//   mr_actual:   Actual number of rows for this micro-kernel, handling M-tails.
//   nr_actual:   Actual number of columns for this micro-kernel, handling N-tails (for this block, max NR_REG_BLOCK_AVX2).
//   is_first_k_block: True if this is the first iteration of the K-block loop for this C tile,
//                     meaning C accumulators should be initialized to zero. Otherwise, load existing C values.
void gemm_avx2_micro_kernel(const float* A_block_ptr, int A_block_ld, 
    ...
    // C accumulators: MR_AVX2 rows, each storing NC_VECS_AVX2 __m256 vectors (total NR_REG_BLOCK_AVX2 floats).
    ...
    for (int r = 0; r < MR_AVX2; ++r) {
        // Always zero out the entire accumulator registers first.
        // This ensures that non-active lanes (due to `nr_actual` or `r >= mr_actual`) are always zero.
        for (int v_idx = 0; v_idx < NC_VECS_AVX2; ++v_idx) {
            c_acc[r][v_idx] = _mm256_setzero_ps();
        }

        if (r < mr_actual) { // Only load from C for valid rows in current C micro-block
            if (!is_first_k_block) {
                for (int v_idx = 0; v_idx < NC_VECS_AVX2; ++v_idx) {
                    // Calculate pointer to the start of the current C vector in the current C row for this micro-kernel.
                    float* current_C_element_ptr = C_ptr_at_tile + (m_offset_in_tile + r) * C_full_ld + n_offset_in_tile + v_idx * VEC_F_AVX2;
    ...
    for (int k_idx = 0; k_idx < k_block_len; k_idx += UNROLL_K) {
        ...
        for (int uk = 0; uk < UNROLL_K; ++uk) {
            ...
            // Prefetch for B_block_ptr: Prefetch the B data for a future K-iteration within the micro-kernel.
            // This aims to bring the data into L1/L2 cache before it's strictly needed.
            if (k_loop_idx + PREFETCH_DISTANCE_K_MICRO < k_block_len) {
                const float* prefetch_b_row_ptr = &B_block_ptr[(k_loop_idx + PREFETCH_DISTANCE_K_MICRO) * B_block_ld + n_offset_in_tile];
            ...
            for (int v_idx = 0; v_idx < NC_VECS_AVX2; ++v_idx) {
                b_vecs[v_idx] = _mm256_maskload_ps(&B_block_ptr[k_loop_idx * B_block_ld + n_offset_in_tile + v_idx * VEC_F_AVX2], load_store_masks_avx2[v_idx]);
            }

            // Perform MR_AVX2 Fused Multiply-Add (FMA) operations for each row.
            for (int r = 0; r < MR_AVX2; ++r) {
                if (r < mr_actual) { // Only for valid M rows
                    // Load A scalar: A_block_ptr[m_offset_in_tile+r][k_loop_idx]
                    // From the packed A block, the offset for the scalar is (m_offset_in_tile + r) * A_block_ld + k_loop_idx.
                    float a_val = A_block_ptr[(m_offset_in_tile + r) * A_block_ld + k_loop_idx];
    ...
    for (int m_block_start = 0; m_block_start < M; m_block_start += BM) {
        for (int n_block_start = 0; n_block_start < N; n_block_start += BN) {
            ...
            for (int k_block_start = 0; k_block_start < K; k_block_start += BK) {
                ...
                // Using _MM_HINT_T0 to suggest prefetching to L1 cache.
                if (k_block_start + BK < K) {
                    const float* next_A_k_block_start_ptr = &A[m_block_start * lda + (k_block_start + BK)];
                    const float* next_B_k_block_start_ptr = &B[(k_block_start + BK) * ldb + n_block_start];
                    
                    // Prefetch a few rows/cache lines from the next A block
                    for(int i = 0; i < std::min(current_BM, 4); ++i) { 
                        _mm_prefetch((const char*)&next_A_k_block_start_ptr[i * lda], _MM_HINT_T0); 
                    }
                    // Prefetch a few rows/cache lines from the next B block
                    for(int i = 0; i < std::min(current_BK, 4); ++i) { 
                        _mm_prefetch((const char*)&next_B_k_block_start_ptr[i * ldb], _MM_HINT_T0);
                    }
                }

                // Pack current A block (current_BM x current_BK) into contiguous memory.
                // This eliminates potential L1/L2 cache misses due to large 'lda' stride
                // and allows for more efficient sequential memory access.
                for (int i = 0; i < current_BM; ++i) {
                    std::copy(&A[(m_block_start + i) * lda + k_block_start],
                              &A[(m_block_start + i) * lda + k_block_start + current_BK],
                              &A_packed[i * current_BK]);
                }

                // Pack current B block (current_BK x current_BN) into contiguous memory.
                // This eliminates potential L1/L2 cache misses due to large 'ldb' stride
                // and allows for more efficient sequential memory access.
                for (int i = 0; i < current_BK; ++i) {
                    std::copy(&B[(k_block_start + i) * ldb + n_block_start],
                              &B[(k_block_start + i) * ldb + n_block_start + current_BN],
                              &B_packed[i * current_BN]);
                }
                ...
                for (int m_local = 0; m_local < current_BM; m_local += MR_AVX2) {
                    ...
                    for (int n_local = 0; n_local < current_BN; n_local += NR_REG_BLOCK_AVX2) {
                        ...
                        gemm_avx2_micro_kernel(A_packed, current_BK,
                                               B_packed, current_BN,
                                               C + m_block_start * ldc + n_block_start, ldc, // Pass base pointer of C tile
...
// AVX-512 micro-kernel: Computes a (MR_AVX512 x NR_REG_BLOCK_AVX512) block of C.
// Same parameters as AVX2 micro-kernel, adapted for AVX-512 types.
void gemm_avx512_micro_kernel(const float* A_block_ptr, int A_block_ld, 
                              const float* B_block_ptr, int B_block_ld, 
                              float* C_ptr_at_tile, int C_full_ld,
                              int m_offset_in_tile, int n_offset_in_tile,
                              int k_block_len,
    ...
    for (int r = 0; r < MR_AVX512; ++r) {
        // Always zero out the entire accumulator registers first.
        for (int v_idx = 0; v_idx < NC_VECS_AVX512; ++v_idx) {
            c_acc[r][v_idx] = _mm512_setzero_ps();
        }

        if (r < mr_actual) { // Only load from C for valid rows in current C micro-block
            if (!is_first_k_block) {
                for (int v_idx = 0; v_idx < NC_VECS_AVX512; ++v_idx) {
                    float* current_C_element_ptr = C_ptr_at_tile + (m_offset_in_tile + r) * C_full_ld + n_offset_in_tile + v_idx * VEC_F_AVX512;
        ...
        for (int uk = 0; uk < UNROLL_K; ++uk) {
            ...
            for (int v_idx = 0; v_idx < NC_VECS_AVX512; ++v_idx) {
                b_vecs[v_idx] = _mm512_mask_loadu_ps(_mm512_setzero_ps(), load_store_masks_avx512[v_idx], 
                                                    &B_block_ptr[k_loop_idx * B_block_ld + n_offset_in_tile + v_idx * VEC_F_AVX512]);
            }
            ...
            for (int k_block_start = 0; k_block_start < K; k_block_start += BK) {
                ...
                for (int m_local = 0; m_local < current_BM; m_local += MR_AVX512) {
                    ...
                    for (int n_local = 0; n_local < current_BN; n_local += NR_REG_BLOCK_AVX512) {
                        ...
                        gemm_avx512_micro_kernel(A_packed, current_BK,
                                                 B_packed, current_BN,
                                                 C + m_block_start * ldc + n_block_start, ldc,
                                                 m_local, n_local,
                                                 current_BK,
                                                 mr_actual, nr_actual,
                                                 k_block_start == 0);
...
void gemm(const float* A, const float* B, float* C,
    ...
    if (selected_kernel == nullptr) {
        ...
        #endif
            { // This block is the final fallback for scalar. It's always reachable.
...
int main(int argc, char* argv[]) {
    ...
    if (run_autotune) {
        ...
        // Expanded candidates list to give autotuner more options.
        std::vector<int> bm_candidates = {64, 96, 128, 192}; 
        std::vector<int> bn_candidates = {64, 128, 192, 256}; 
        std::vector<int> bk_candidates = {128, 256, 384, 512}; 
