...
// On Zen architectures, 8 might provide better FMA unit utilization by hiding latency more effectively,
// without excessive register pressure (Zen 3 has ample FP registers).
...
void gemm_avx2(const float* A, const float* B, float* C,
    ...
    // `packed_B_buffer` is allocated once per thread inside this region to avoid false sharing
    // and expensive repeated allocations/deallocations.
    #pragma omp parallel
    {
        ...
        // Changed `schedule(static)` to `schedule(dynamic, 1)` (in this iteration 256:0) to potentially improve load
        // balancing, especially for non-perfectly divisible matrix sizes or varying block
        // execution times, by distributing tasks one by one as threads become available.
            ...
            for (int bn_start = 0; bn_start < N; bn_start += BN) {
                ...
                for (int m_tile_start = bm_start; m_tile_start < bm_start + current_BM; m_tile_start += MR) {
                    ...
                    for (int n_tile_start = 0; n_tile_start < current_BN; n_tile_start += NR) {
                        ...
                        // Initialize C accumulators ONCE for this MR x NR micro-block.
                        // They will accumulate contributions from ALL K-blocks, ensuring full GEMM computation.
                        ...
                        for (int bk_start = 0; bk_start < K; bk_start += BK) {
                            ...
                            for (int k_micro = 0; k_micro < current_BK; k_micro += UNROLL_K) {
                                ...
                                for (int u = 0; u < current_UNROLL_K; ++u) {
                                    ...
                                    // Use masked load for B-vectors in N-tail regions.
                        ...
                        // Store the accumulated C values ONCE after all K contributions are processed.
                        for (int r = 0; r < current_MR; ++r) {
        ...
        // Free the per-thread aligned memory when the thread exits the parallel region.
...
int main(int argc, char* argv[]) {
    ...
    if (dump_matrices) {
        ...
        (void)system("mkdir -p workspace"); // Suppress unused result warning
