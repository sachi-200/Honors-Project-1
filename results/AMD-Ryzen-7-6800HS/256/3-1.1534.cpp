...
// can ideally fit into L2/L3 caches.
//
// For Ryzen 7 6800HS (Zen 3+):
// L1d cache per core: 32 KB.
// L2 cache per core: 512 KB.
//
// Based on profiling data showing high L1d cache misses, the blocking parameters have been
// adjusted to better fit the working set within the 32KB L1d cache.
//
// New chosen values are BM=64, BN=64, BK=32:
// - A-block (BM x BK): 64 rows * 32 cols * 4 bytes/float = 8192 bytes (~8 KB).
// - B-block (BK x BN): 32 rows * 64 cols * 4 bytes/float = 8192 bytes (~8 KB).
// - C-block (BM x BN): 64 rows * 64 cols * 4 bytes/float = 16384 bytes (~16 KB).
//
// The total working set for one K-block iteration (A_block + B_block + C_block) is approximately
// 8KB + 8KB + 16KB = 32KB. This combined size fits perfectly within the 32KB L1d cache per core,
// which should significantly improve L1 data locality and reduce cache misses.
// The overall block sizes (BM, BN, BK) are also small enough to facilitate good L2 cache reuse.
...
// This small footprint stays entirely in registers, providing maximal data reuse.
constexpr int VEC_WIDTH_AVX2 = 8; // __m256 holds 8 floats
constexpr int MR_AVX2 = 6;        // Number of rows of A/C to process in the micro-kernel (e.g., 6 rows of C)
...
void gemm_avx2(const float* A, const float* B, float* C,
    ...
    // which is generally good for predictable performance on uniform workloads.
            ...
            for (int kb = 0; kb < K; kb += BK) { // Loop over K-blocks (inner dimension)
                ...
                for (int i = mb; i < M_block_end; i += MR) { // Loop over micro-kernel rows of C
                    ...
                    for (int j = nb; j < N_block_end; j += NR) { // Loop over micro-kernel columns of C
                        ...
                        for (int r = 0; r < MR; ++r) {
                            if (i + r < i_micro_end) { // Check if this row is within the M-block's valid range
                                if (kb == 0) {
                                    C_acc[r] = _mm256_setzero_ps();
                                } else {
                                    // _mm256_loadu_ps loads 8 floats from memory, unaligned.
                                    // This is safe as long as the memory is allocated and readable.
                                    // The main function ensures memory is zero-initialized and sufficiently large.
                        ...
                        // UNROLL_K determines how many K steps are processed in one iteration, reducing loop overhead.
                        for (int p = kb; p < K_block_end; p += UNROLL_K) {
                            for (int p_unroll = 0; p_unroll < UNROLL_K && p + p_unroll < K_block_end; ++p_unroll) {
                                // Prefetching: Hints to the CPU to bring data into the cache hierarchy.
                                // _MM_HINT_T0 requests data to L1 cache. This attempts to prefetch
                                // elements from A and B for the next set of K-unroll iterations.
                                ...
                                for (int r = 0; r < MR; ++r) {
                                    ...
                                    // Load A[i_row][p_col] and broadcast its scalar value to all 8 elements of an __m256 register.
                                    // This is efficient as 'A_broadcast' acts as the scalar multiplier for a vector 'B_vec'.
                                    __m256 A_broadcast = _mm256_set1_ps(A[static_cast<size_t>(i + r) * lda + (p + p_unroll)]);
                                    
                                    // Load B[p_col][j_col:j_col+NR] as a vector.
                                    // `_mm256_loadu_ps` is used for unaligned loads, which is safe for arbitrary `ldb` and `j` offsets.
                                    ...
                                    // This instruction performs a multiplication and an addition in a single operation,
                                    // improving throughput and precision (by doing the intermediate multiplication with higher precision).
                        ...
                        for (int r = 0; r < MR; ++r) {
                            if (i + r < i_micro_end) { // Ensure row is within M-block
                                ...
                                if (cols_to_store >= VEC_WIDTH) { // Full vector store if enough columns
                                    ...
                                    // For AVX2, there isn't a direct masked store for floats as flexible as AVX-512.
                                    // The robust approach is to store the full vector to a temporary aligned buffer
                                    // and then scalar copy only the valid elements.
                                    alignas(32) float temp_vec[VEC_WIDTH];
                                    _mm256_store_ps(temp_vec, C_acc[r]); // Store full vector to temp (aligned store is fine)
                                    for (int col_idx = 0; col_idx < cols_to_store; ++col_idx) {
                                        C_ptr[col_idx] = temp_vec[col_idx]; // Copy only valid elements
                                    }
                                }
                                // If cols_to_store is 0 or negative, no columns in this micro-block are valid, so do nothing.
                            }
                        }
...
void gemm_avx2(const float* A, const float* B, float* C,
               int M, int N, int K,
               int lda, int ldb, int ldc) {
    std::cerr << "Warning: AVX2 kernel not compiled. Falling back to scalar." << std::endl;
            ...
            for (int kb = 0; kb < K; kb += BK) {
                ...
                for (int i = mb; i < M_block_end; i += MR) {
                    ...
                    for (int j = nb; j < N_block_end; j += NR) {
                        ...
                        // `__mmask16` is a 16-bit mask register, where each bit controls one float lane.
                        // `(1 << N)` creates a bitmask where bits 0 to N-1 are set.
                        // If `j_micro_end - j` is 16, mask is `(1 << 16) - 1` (0xFFFF), which is all 1s.
                        // If it's less than 16, only the relevant number of bits are set.
                        __mmask16 n_mask = static_cast<__mmask16>((1 << (j_micro_end - j)) - 1);
                        if (j_micro_end - j <= 0) n_mask = 0; // Ensure mask is zero if no columns to process.
                        ...
                        for (int r = 0; r < MR; ++r) {
                            if (i + r < i_micro_end) {
                                if (kb == 0) { // First K-block: initialize accumulators to zero
                                    ...
                                    // without requiring scalar loops or temporary buffers.
                        ...
                        for (int r = 0; r < MR; ++r) {
                            if (i + r < i_micro_end) {
                                ...
                                // This handles column tails efficiently using hardware masks.
...
void gemm_avx512(const float* A, const float* B, float* C,
                 int M, int N, int K,
                 int lda, int ldb, int ldc) {
    std::cerr << "Warning: AVX-512 kernel not compiled. Falling back to AVX2 or scalar." << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    size_t size_B = static_cast<size_t>(K) * N; // Declared size_B
    size_t size_C = static_cast<size_t>(M) * N; // Declared size_C
