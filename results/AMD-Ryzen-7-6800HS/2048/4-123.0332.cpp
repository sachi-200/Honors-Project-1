...
// UNROLL_K=8 is chosen in this iteration to further expose Instruction-Level Parallelism (ILP)
// to the CPU's out-of-order execution engine. Given that the workload is still identified
// as Compute Bound, increasing unrolling can help keep the FMA units saturated and reduce
// loop overhead, which is beneficial on modern architectures like Zen 3.
    ...
    for (int m_block_start = 0; m_block_start < M; m_block_start += BM) {
        for (int n_block_start = 0; n_block_start < N; n_block_start += BN) {
            ...
            for (int k_block_start = 0; k_block_start < K; k_block_start += BK) {
                ...
                // It also zero-pads rows if current_N_block_size < BN, and implicitly pads
                // rows if current_K_block_size < BK (remaining B_packed_local rows are left zeroed).
